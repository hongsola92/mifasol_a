...
- 머신러닝
    - 데이터 전처리
        ...
        - 이상치 처리하면 더 정확해 진다.
        - 정규화 해야 한다.
            - 정규화 하는 2가지 방법 : z-score /  정규화
    - 회귀 이론
        - 방정식(최소 작용법), 반복접근(최소 제곱법)
            - 반복접근을 해서 모델에 변수를 찾는 역할을 함.
----------------------------------
- 경사하강법
    - 전체-배치 경사하강법
        - 업데이트 횟수 감소
    ...
    - 확률적 경사하강법
    - 미니-배치 경사하강법
        - 에포크(epoch) : 데이터를 한 번에 모두 학습시키는 횟수를 의미.
        - 배치 사이즈(batch-size) : 한 번에 학습이 되는 데이터의 개수를 의미.

## 024-06-12
### 복습
- 선형회귀
    - 1. 방정식 풀기 : 빠르고 중간과정이 없다.
    - 2. 차근차근 접근하는 방법 : 경사 하강법 (GD, SGD, Mini batch SGD) + 하이퍼 파라미터(lr(learning rate))
- 정규화
    - L1 또는 L2 방식으로 정규화를 할 수 있다.
---------------------------
- 베이즈 정리의 이해
    - 확률의 표현
        - 확률의 기본 성질 : 모든 사건에 대해 반드시 0에서 1사이의 값을 가져야 함.
        - 조건부 확률 : 어떤 사건이 일어난다고 가정했을 때 다른 사건이 일어날 확률을 의미.
    - 베이즈 정리: 두 확률 변수의 사전확률과 사후확률 사이의 관계를 나타내는 정리 

## 024-06-18
### C
- 자료구조(data structure) = (data + 관련된 연산)
    - pg = data + 명령
